...
namespace NXAI.Infra.Repository.EfCore.Transaction;

public abstract class UnitOfWork<TDbContext>(TDbContext context, ILogger<UnitOfWork<TDbContext>>? logger) : IUnitOfWork
    where TDbContext : DbContext
{
    protected TDbContext NXAIDbContext { get; init; } = context ?? throw new ArgumentNullException(nameof(context));

    protected IDbContextTransaction? DbTransaction { get; set; }

    public bool IsStartingUow => NXAIDbContext.Database.CurrentTransaction is not null;

    public void BeginTransaction(IsolationLevel isolationLevel = IsolationLevel.ReadCommitted, bool distributed = false)
    {
        if (NXAIDbContext.Database.CurrentTransaction is not null)
        {
            throw new InvalidDataException($"UnitOfWork Error,{NXAIDbContext.Database.CurrentTransaction}");
        }
        else
        {
            NXAIDbContext.Database.AutoSavepointsEnabled = false;
