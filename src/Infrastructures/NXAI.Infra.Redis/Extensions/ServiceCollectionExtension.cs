...
using NXAI.Infra.Redis;
using NXAI.Infra.Redis.Configurations;
using NXAI.Infra.Redis.Core;
using NXAI.Infra.Redis.Core.Serialization;
using StackExchangeProvider = NXAI.Infra.Redis.Providers.StackExchange;

namespace Microsoft.Extensions.DependencyInjection;

public static class ServiceCollectionExtension
{
    public static IServiceCollection AddNXAIInfraRedis(this IServiceCollection services, IConfigurationSection redisSection, ServiceLifetime serviceLifetime = ServiceLifetime.Scoped)
    {
        ArgumentNullException.ThrowIfNull(services, nameof(services));
        ArgumentNullException.ThrowIfNull(redisSection, nameof(redisSection));

        if (services.HasRegistered(nameof(AddNXAIInfraRedis)))
        {
            return services;
        }
